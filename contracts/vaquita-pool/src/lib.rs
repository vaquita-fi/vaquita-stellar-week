#![no_std]
use soroban_sdk::{
    contract, contractimpl, vec, contracttype, Address, Env, String, Vec, Symbol, token::Client as TokenClient
};
...
impl VaquitaPool {
    // ---------- Initialization ----------
    pub fn initialize(env: Env, admin: Address, token: Address, blend_token: Address, pool_address: Address, defindex_vault_address: Address, lock_periods: Vec<u64>) {
    ...
    pub fn deposit(env: Env, caller: Address, deposit_id: String, amount: i128, period: u64) {
        ...
        let defindex_vault_address = env.storage().instance().get(&DataKey::DeFindexVaultAddress).unwrap();
        ...
        // Calculate b_rate at the moment of depositing from Blend Pool
        ...
        // Update total deposits for this period
    ...
    pub fn withdraw(env: Env, caller: Address, deposit_id: String) {
        ...
        // Withdraw from DeFindex with the correct amount
